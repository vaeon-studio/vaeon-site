// =====================================================
// APPLY-CONFIG — moteur d'injection (Beauté Iconique)
...
(() => {
  ...
  const apply = () => {
    ...
    if (se.actif !== false && expSection) {
      ...
      setText('[data-cfg="experience-brand"]',    se.marque    || 'MAISON LUMIÈRE');
      ...
      if (titleEl) {
        const t1 = escapeHtml(se.titre || 'L\'EXPÉRIENCE');
        const t2 = escapeHtml(se.titreSousLigne || 'MAISON LUMIÈRE');
    ...
    if (ft.actif !== false && footerEl) {
      ...
      // Nom du salon (bottom bar — "Maison Lumière" italique)
      setText('.footer-brand', ft.nomCommerce || C.nomCommerce || 'Maison Lumière');
      ...
      setText('.footer-copyright', ft.copyright || '© Maison Lumière');
