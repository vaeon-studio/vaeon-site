// === Particle Text Effect v2 — cycling words + interactive hover dispersion ===
// Original cycle behavior (KAINXU concept) + new hover repulsion (cursor pushes nearby particles).
...
class Particle {
  ...
  // NEW: apply outward push from cursor (additive on top of steer-to-target)
...
function initParticleText(canvas, words, opts = {}) {
  ...
  // Hover repulsion config
  const hoverRadius = opts.hoverRadius || 110;
  const hoverStrength = opts.hoverStrength || 6;
  ...
  function tick() {
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
      const p = particles[i];
      // NEW: hover repulsion (only when mouse is over canvas, not just pressed)
      if (mouse.isHovering) p.repel(mouse.x, mouse.y, hoverRadius, hoverStrength);
      p.move();
